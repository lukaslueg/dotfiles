...
alias fedora_clear_leaves='sudo dnf remove $(peco <(for pkgname in `package-cleanup --all --leaves -q | sort`; [[ ! -n `grep -m1 $(rpm -q --queryformat "^%{NAME}$" $pkgname | head -n1) $HOME/.dotfiles/fedora_worldfile` ]] && echo $pkgname;))'
